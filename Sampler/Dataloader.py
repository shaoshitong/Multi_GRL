...
def get_data_deap(sample_path, subject_number, i, num_classes,batchsize=128):
    ...
    print("source 2:{:.2f}%,1:{:.2f}%,0:{:.2f}%".format(
          np.sum(source_elabel == 2)/l2,
          np.sum(source_elabel == 1)/l2,
          np.sum(source_elabel == 0)/l2))
    source = TrainDataset(source_sample, source_elabel,i,subject_number,num_classes,training=True,name="deap")
    target = TrainDataset(source_sample, source_elabel,i,subject_number,num_classes,training=False,name="deap")
